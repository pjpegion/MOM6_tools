...
bathy_file='/scratch2/BMC/gsienkf/Philip.Pegion/UFS-coupled/quarter_deg_basedir/INPUT/topog.nc'
edits_file='/scratch2/BMC/gsienkf/Philip.Pegion/UFS-coupled/quarter_deg_basedir/INPUT/All_edits.nc'
...
grid_file_out=xr.open_dataset(wgt_dir+'tripole.mx025.nc')
wgtsfile_t_to_t = wgt_dir+'ORAS5.Ct.to.mx025.Ct.bilinear.nc'
wgtsfile_t_to_u = wgt_dir+'tripole.mx025.Ct.to.Cu.bilinear.nc'
wgtsfile_t_to_v = wgt_dir+'tripole.mx025.Ct.to.Cv.bilinear.nc'
...
mx025_t_grid=grid_file_out.rename({'lonCt': 'lon', 'latCt': 'lat'})
mx025_v_grid=grid_file_out.rename({'lonCv': 'lon', 'latCt': 'lat'})
mx025_u_grid=grid_file_out.rename({'lonCu': 'lon', 'latCu': 'lat'})

# rotation angles for u and v currents
ang_out= grid_file_out.anglet

rg_tt = xe.Regridder(oras5_t_grid, mx025_t_grid, 'bilinear',periodic=True,reuse_weights=True, filename=wgtsfile_t_to_t)
rg_tu = xe.Regridder(mx025_t_grid, mx025_u_grid, 'bilinear',periodic=True,reuse_weights=True, filename=wgtsfile_t_to_u)
rg_tv = xe.Regridder(mx025_t_grid, mx025_v_grid, 'bilinear',periodic=True,reuse_weights=True, filename=wgtsfile_t_to_v)
...
for i in range(nz-1):
    new_t[0,i+1]=np.where(new_bathy > oras5.depth[i].values,new_t[0,i+1],new_t[0,i])
    new_s[0,i+1]=np.where(new_bathy > oras5.depth[i].values,new_s[0,i+1],new_s[0,i])
    new_u[0,i+1]=np.where(new_bathy > oras5.depth[i].values,new_u[0,i+1],new_u[0,i])
    new_v[0,i+1]=np.where(new_bathy > oras5.depth[i].values,new_v[0,i+1],new_v[0,i])
...
#generate new_h
h1d=np.zeros(nz)
h1d[0:-1]=oras5.depth[1:].values-oras5.depth[0:-1].values
h1d[-1]=h1d[-2]
